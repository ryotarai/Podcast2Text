...
class ScriptsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create script" do
    assert_difference('Script.count') do
      post scripts_url, params: { script: { audio_url: @script.audio_url, text: @script.text, title: @script.title } }
    end
  ...
  test "should update script" do
    patch script_url(@script), params: { script: { audio_url: @script.audio_url, text: @script.text, title: @script.title } }
